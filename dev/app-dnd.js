/* ============================================
   ...
   *** DEV 프로토타입 (_dev/app-dnd.js) ***
   원본 app.js를 복사한 뒤 수동 매칭에 드래그 앤 드롭을 추가한 실험용 파일.
   실제 배포본에 반영하지 않음. 아래 두 가지를 원본과 다르게 설정해 운영 데이터를 건드리지 않도록 함:
   - APPS_SCRIPT_URL을 비워 구글시트로의 저장/내보내기(POST)를 전부 차단 (참가자 목록 조회 등 읽기는 그대로 동작)
   - STORAGE_KEY를 별도 키로 분리해 실제 앱의 localStorage 캐시를 건드리지 않음
   ============================================ */
...
const CONFIG = {
    ...
    ATT_LOG_TAB: '출석기록',
    STATE_TAB: '상태',
    APPS_SCRIPT_URL: '', // DEV: 저장/내보내기(POST)로 실제 시트가 바뀌지 않도록 비워둠
    DEFAULT_COURTS: 3,
...
function getLastGameTypes(name, count = 5) {
    const types = [];
    for (let i = S.gameLog.length - 1; i >= 0 && types.length < count; i--) {
        const g = S.gameLog[i];
        if ([...g.teamA, ...g.teamB].some(p => p.name === name)) types.push(g.type);
    }
    return types;
}
...
function renderPlayers() {
    ...
    if (!arr.length) {
        ...
        list.innerHTML = header + header.replace('pr-header', 'pr-header pr-header-dup') + arr.map(p => {
            ...
            return `
            <div class="pr ${p.selected?'selected':''} ${p.status} ${isQueued?'queued':''} gender-${genderCls}" data-pid="${p.id}" onclick="toggleSelect('${p.id}')">
                <span class="pr-main">
        ...
        <div class="preview-game" ${genderWarning ? 'style="border-color:var(--amber)"' : ''}>
            ...
            <div class="preview-teams">
                <div class="preview-team">
                    ...
                    ${split.teamA.map(p=>`<div class="team-player" data-pid="${p.id}" title="드래그하면 선택 해제"><span class="lv lv-${p.level}"></span>${p.name} <span style="color:var(--txt3);font-size:.68rem">${p.level}</span></div>`).join('')}
                </div>
                <div class="preview-vs">VS</div>
                <div class="preview-team">
                    <div class="preview-team-label">TEAM B (${sB})</div>
                    ${split.teamB.map(p=>`<div class="team-player" data-pid="${p.id}" title="드래그하면 선택 해제"><span class="lv lv-${p.level}"></span>${p.name} <span style="color:var(--txt3);font-size:.68rem">${p.level}</span></div>`).join('')}
                </div>
            </div>
...
const STORAGE_KEY = 'vertical_badminton_state_v1_dnd_dev'; // DEV: 실제 앱 캐시와 분리
...
/* ============================================
   DEV: 수동 매칭 드래그 앤 드롭
   ============================================
   기존 클릭 선택(toggleSelect)은 그대로 둔 채, 포인터 이벤트로 드래그 동작만 얹는다.
   - 대기 인원 카드 · 매칭 미리보기의 팀 카드(data-pid가 붙은 요소) 모두 드래그 가능
   - "게임 매칭" 패널(.panel-match) 위에서 놓으면 → 선택(toggleSelect의 select-only 버전)
   - 그 외 지점에서 놓으면 → 선택 해제 (원래 선택돼 있지 않았다면 아무 일도 없음)
   - 클릭(드래그 없이 떼기)은 기존 onclick="toggleSelect(...)" 그대로 동작하도록,
     실제로 포인터가 일정 거리 이상 움직였을 때만 드래그로 간주하고 뒤이은 click을 막는다.
   ============================================ */
let _dndSuppressClick = false;

function initManualMatchDnd() {
    const DRAG_THRESHOLD = 6; // px
    let drag = null; // { pid, startX, startY, moved, ghost }

    // 드래그 직후 브라우저가 자동으로 발생시키는 click 이벤트 1회를 무시.
    // renderPlayers()/renderPreview()가 pointerup 핸들러 안에서 카드를 재생성하므로,
    // 원래 카드에 대한 참조로는 뒤이은 click의 대상을 특정할 수 없어 전역 플래그로 처리한다.
    document.addEventListener('click', (e) => {
        if (!_dndSuppressClick) return;
        _dndSuppressClick = false;
        e.stopPropagation();
        e.preventDefault();
    }, true);
    ...
    document.addEventListener('pointerdown', (e) => {
        ...
        const card = e.target.closest('.pr[data-pid], .team-player[data-pid]');
        if (!card) return;
        // 카드 안의 다른 컨트롤(체크박스, 상태 select, 삭제버튼 등)에서 시작한 드래그는 무시
        if (e.target.closest('.pr-shuttle, .pr-status-select, .pr-del, .pr-lastgame')) return;

        const p = S.players.find(x => x.id === card.dataset.pid);
        if (!p) return;

        drag = { pid: p.id, startX: e.clientX, startY: e.clientY, moved: false, ghost: null, card };
    });
    ...
    document.addEventListener('pointermove', (e) => {
        ...
        if (!drag.moved) {
            drag.moved = true;
            drag.ghost = buildDndGhost(drag.card);
            document.body.appendChild(drag.ghost);
            drag.card.classList.add('dnd-dragging');
        }
    ...
    document.addEventListener('pointerup', (e) => {
        if (!drag) return;
        const { pid, moved, card, ghost } = drag;

        if (moved) {
            ghost.style.display = 'none';
            const overTarget = document.elementFromPoint(e.clientX, e.clientY);
            ghost.remove();
            card.classList.remove('dnd-dragging');
            clearDndDropTarget();

            const droppedOnMatchPanel = !!(overTarget && overTarget.closest('.panel-match'));
            if (droppedOnMatchPanel) dndSelect(pid);
            else dndDeselect(pid);

            // 드래그였던 클릭은 한 번 무시 (toggleSelect 중복 토글 방지)
            _dndSuppressClick = true;
        }
        drag = null;
    });
    ...
    document.addEventListener('pointercancel', () => {
        if (!drag) return;
        drag.ghost?.remove();
        drag.card?.classList.remove('dnd-dragging');
        clearDndDropTarget();
        drag = null;
    });
}

function buildDndGhost(card) {
    const ghost = card.cloneNode(true);
    ghost.className = card.className.replace('dnd-dragging', '') + ' dnd-ghost';
    ghost.removeAttribute('id');
    ghost.style.width = card.offsetWidth + 'px';
    return ghost;
}

function updateDndDropTarget(x, y) {
    const el = document.elementFromPoint(x, y);
    const panel = document.querySelector('.panel-match');
    if (!panel) return;
    const over = !!(el && el.closest('.panel-match'));
    panel.classList.toggle('dnd-drop-target', over);
}

function clearDndDropTarget() {
    document.querySelector('.panel-match')?.classList.remove('dnd-drop-target', 'dnd-drop-invalid');
}

/** 드래그로 "게임 매칭" 패널에 놓았을 때: 선택 상태로 만든다 (이미 선택돼 있으면 아무 일도 안 함) */
function dndSelect(pid) {
    const p = S.players.find(x => x.id === pid);
    if (!p || p.status === 'playing' || p.status === 'late') return;
    if (S.queue.some(g => g.teamA.includes(pid) || g.teamB.includes(pid))) {
        toast('이미 대기게임에 편성된 인원입니다', 'info');
    ...
    if (p.selected) return; // 이미 선택됨
    if (p.status === 'resting') p.status = 'waiting';
    p.selected = true;
    S.selectedIds.push(pid);
    renderPlayers(); renderPreview();
}

/** 드래그로 패널 밖에 놓았을 때: 선택 해제 (선택돼 있지 않았다면 변화 없음) */
function dndDeselect(pid) {
    const p = S.players.find(x => x.id === pid);
